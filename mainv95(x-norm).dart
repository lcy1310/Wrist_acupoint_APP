...
const bool kSwapDisplayRotation90And270 = false;
...
class YuvToRgbPreprocessor {
  static FrameTensor preprocess(
    ...
    final double scale = math.min(inputSize / width, inputSize / height);
    final int resizedW = (width * scale).round();
    final int resizedH = (height * scale).round();
    ...
    for (int dy = 0; dy < inputSize; dy++) {
      ...
      final int sy = insideY ? (((dy - padY) / scale).floor()).clamp(0, height - 1) : 0;
      ...
      for (int dx = 0; dx < inputSize; dx++) {
        ...
        final int sx = (((dx - padX) / scale).floor()).clamp(0, width - 1);
        final int srcX = mirrorX ? (width - 1 - sx) : sx;
        final int yIndex = sy * yRowStride + srcX;
        final int uvIndexU = (sy >> 1) * uRowStride + (srcX >> 1) * uPixelStride;
        final int uvIndexV = (sy >> 1) * vRowStride + (srcX >> 1) * vPixelStride;
...
class AcupointOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final logicalSource = (orientation == 90 || orientation == 270)
    ...
    if (kDebugInferenceLogs) {
      ...
      final signature =
          '${source.width}x${source.height}_${logicalSource.width}x${logicalSource.height}_${destination.left.toStringAsFixed(1)}_'
          '${destination.top.toStringAsFixed(1)}_${destination.width.toStringAsFixed(1)}_${destination.height.toStringAsFixed(1)}_'
          '${orientation}_${preview.dx.toStringAsFixed(1)}_${preview.dy.toStringAsFixed(1)}';
      if (_OverlayDebugCache.shouldPrint(signature)) {
        debugPrint(
          'OVERLAY source=$source logicalSource=$logicalSource size=$size '
          'dest=${destination.left.toStringAsFixed(1)},${destination.top.toStringAsFixed(1)},'
          '${destination.width.toStringAsFixed(1)}x${destination.height.toStringAsFixed(1)} orientation=$orientation',
        );
      }
    }

    Offset rotateForDisplay(Offset p) {
      switch (orientation) {
        case 90:
          return kSwapDisplayRotation90And270
              ? Offset(p.dy, source.width - p.dx)
              : Offset(source.height - p.dy, p.dx);
        case 180:
          return Offset(source.width - p.dx, source.height - p.dy);
        case 270:
          return kSwapDisplayRotation90And270
              ? Offset(source.height - p.dy, p.dx)
              : Offset(p.dy, source.width - p.dx);
    ...
    Offset mapPoint(Offset raw) {
      // Map from raw sensor frame coordinates to the actually displayed preview.
      final display = rotateForDisplay(raw);
      final sx = destination.width / logicalSource.width;
      final sy = destination.height / logicalSource.height;
      return Offset(
        destination.left + display.dx * sx,
        destination.top + display.dy * sy,
      );
    }
